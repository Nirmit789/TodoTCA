...
/*
 This struct represent main state of App. When there is some change in app State it get progated to Sub states. Here in this case any change in Todos state will be prpogated to todo and ui will be updated accordingly
 
 Action enum signifies the all possible action that can happen in the app which can update the state of app.
 
 Reducer is the brain which check which action it is and update the state. Sometime data needs to come from Api or other source then we create the effect which will again call some action asynchronously which in result update the state. In below struct the reducer logic is variable body of ReducerProtocol.
 */
